...
if ($arParams['FIND_BY_UF_NAME'] == 'Y' && isset($arParams['UF_NAME'])) {
	$ufValue = "";
	if($arParams['UF_NAME'] == 'UF_NEW') {
		$ufValue = true;
	}
	else {
		$ufValue = $arParams['SECTION_ID'];
	}
	...
	$rsSections = CIBlockSection::GetList(
		...
		array(
			...
			'ACTIVE' => 'Y',
			'GLOBAL_ACTIVE' => 'Y',
			$arParams['UF_NAME'] => $ufValue
		),
		true,
		array($arParams['UF_NAME'], 'UF_SECTION_DESCR')
	);
	$arSections = array();
	while ($arSection = $rsSections->GetNext()) {
		$arSection['PICTURE'] = (0 < $arSection['PICTURE'] ? CFile::GetFileArray($arSection['PICTURE']) : false);
		array_push($arSections, $arSection);
	}
