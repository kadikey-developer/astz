...
<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="filter mb-60">
	<?/*<div class="smart-filter-title"><?echo GetMessage("CT_BCSF_FILTER_TITLE")?></div>*/?>
			...
			<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5">
				...
				{
					$key = $key . '_' . $arParams["~FILTER_NAME"];
					...
					<?if($iFilterParams == 5):?>
						...
						<div class="collapse" id="collapse-additional-params">
						<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5">
					<?endif;?>
					<div class="col mb-20 mb-lg-60 smart-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active<?endif?>">
						<span class="smart-filter-container-modef"></span>
		
						<div onclick="smartFilter.hideFilterProps(this)">
		
							<label class="ff-dinpro fw-500"><?=$arItem["NAME"]?></label>
							...
							<div class="smart-filter-parameters-box-container">
										...
										<div class="range-slider">
											<div class="form-group">
												<div class="row no-gutters mb-20">
													<div class="col">
														<label class="range-slider__label text-lowercase" for="<?echo $arItem["VALUES"]["MIN"]["CONTROL_ID"]?>"><?=GetMessage("CT_BCSF_FILTER_FROM")?></label>
														<input
															<?/*id="<?echo $arItem["VALUES"]["MIN"]["CONTROL_ID"]?>"*/?>
															id="<?echo $arItem["VALUES"]["MIN"]["CONTROL_ID"]?>"
													...
													<div class="col">
														<label class="range-slider__label text-lowercase" for="<?echo $arItem["VALUES"]["MAX"]["CONTROL_ID"]?>"><?=GetMessage("CT_BCSF_FILTER_TO")?></label>
														<input
															id="<?echo $arItem["VALUES"]["MAX"]["CONTROL_ID"]?>"
								...
								?>
									<div class="lib-select lib-select-sm">
										<?/*foreach ($arItem["VALUES"] as $val => $ar):?>
											<? if ( $ar["CHECKED"] ) print_r("selected");?>
										<?endforeach;*/?>
										<select class="js-lib-select" name="<?=$arCur["CONTROL_NAME_ALT"]?>">
											<option id="<?= "all_".$arCur["CONTROL_ID"] ?>" name="<?=$arCur["CONTROL_NAME_ALT"]?>" value="">Все</option>
											<?foreach ($arItem["VALUES"] as $val => $ar):?>
												<option
													id="<?=$ar["CONTROL_ID"]?>"
													<? if ( $ar["CHECKED"] ) echo "selected";?>
									...
									<div class="smart-filter-input-group-dropdown">
										<div class="smart-filter-dropdown-block" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
											...
											<div class="smart-filter-dropdown-popup lib-select lib-select-sm" data-role="dropdownContent" style="display: none;">
												<div class="js-lib-select">
													<ul>
														<li>
				...
				?>
				<?if($iFilterParams > 5) echo'</div> <!--//row additional-params-->';?>
			</div><!--//row or collapse-->
			<?if($iFilterParams > 5):?>
			<div class="d-flex align-items-center">
				<div class="flex-grow-1 border-bottom"></div>
				<div>
					<a class="collapse-action btn btn-link btn-sm" data-toggle="collapse" href="#collapse-additional-params" role="button" aria-expanded="false" aria-controls="collapse-additional-params">
						<span class="mr-20">Доп. свойства фильтра</span><span class="collapse-caret icon-chevron-bold"></span> 
					</a>
				</div>
				<div class="flex-grow-1 border-bottom"></div>
			</div>
			...
			foreach ( $arResult['ITEMS'] as $id => $item ) {
				foreach ( $item['VALUES'] as $valID => $value ) {
					switch ( $item['DISPLAY_TYPE'] ) {
						case 'F':
							if ( $value['CHECKED'] ) {
								if ( !isset($selectedFilters['ACTIVE_PROPS'][$id]) ) {
									$selectedFilters['ACTIVE_PROPS'][$id] = array(
										'NAME' => $item['~NAME'],
										'URL' => strtolower($item['CODE']), 
										'VALUE' => $value['VALUE'],
									);
								} else {
									$selectedFilters['ACTIVE_PROPS'][$id]['VALUE'] .= ", " . $value['VALUE'];
								}
								$property['PROPERTY_' . $id] = $valID; 
								array_push($selectedFilters['FILTERED_PROPERTIES'], $property);
							}
							break;
						case 'A':
							if ( isset($value['HTML_VALUE']) ) {
								$selectedFilters['ACTIVE_PROPS'][$id] = array(
									'NAME' => $item['~NAME'],
									'URL' => strtolower($item['CODE']),
									'VALUE' => array(
										'MIN' => $item['VALUES']['MIN']['HTML_VALUE'],
										'MAX' => $item['VALUES']['MAX']['HTML_VALUE'],
									) 
								);
								switch ($valID) {
									case 'MAX':
										$property['<=PROPERTY_' . $id] = $value['HTML_VALUE'];
										array_push($selectedFilters['FILTERED_PROPERTIES'], $property);
										break;
									case 'MIN':
										$property['>=PROPERTY_' . $id] = $value['HTML_VALUE']; // попробовать сделать, чтобы только знак сравнения ставился через switch
										array_push($selectedFilters['FILTERED_PROPERTIES'], $property);
										break;
								}
								$selectedFilters['FILTERED_PROPERTIES'] = $property;
							}
							break;
						case 'P':
							if ( isset($value['CHECKED']) ) {
								...
								$property['PROPERTY_' . $id] = $value["HTML_VALUE_ALT"]; 
								array_push($selectedFilters['FILTERED_PROPERTIES'], $property);
							}
							break;
					}
				...
				<div class="mb-60">
					<div class="d-flex flex-column flex-sm-row align-items-sm-center mb-20">
						<div class="mr-20 mb-10 mb-sm-0">
							выбрано: <?= CIBlockElement::GetList(
								array('SORT' => 'ASC'),
								array(
						...
						<div>
							<div class="btn btn-outline-blue-dark h-auto py-5 px-20">
								<input
				...
				<div>
					...
					foreach ($selectedFilters['ACTIVE_PROPS'] as $prop) {
						...
						<div class="d-inline-flex align-items-center mr-30 mb-10">
							...
							<div>
								<? // ссылка на деактивацию параметра
								$regex = '/^(.*)(\/'. $prop["URL"] .'-.*?)(\/.*)$/';
								$clearlink = preg_replace($regex, '$1$3', $arResult['FILTER_URL']);
								$clearlink = preg_replace('/filter\/apply\//', '', $clearlink);
								?>
								<a href="<?=$clearlink?>" data-clear="<?=$clearlink?>" class="text-red"><span class="icon-cross-thin d-inline-block fz-12"></span></a>
							</div>
						</div>
						<?
					}
