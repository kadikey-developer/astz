...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<div class="col-sm-6 col-md-4 mb-30" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
	<div class="border shadow-hover h-100">
		<?if($arParams["DISPLAY_PICTURE"]!="N"):?>
			...
			<?elseif (!is_array($arItem["PREVIEW_PICTURE"])):?>
				<?$arItem["PREVIEW_PICTURE"]["SRC"] = $this->GetFolder() . '/images/no_photo_' . $arItem["PROPERTIES"]["FILES"]["FILE_TYPE"] . '.jpg';?>
			<?endif;?>
				<div class="bx-newslist-img">
				<?if( isset($arFileValue) && $arResult["USER_HAVE_ACCESS"] ):?>
				...
				<?else:?>
					<div class="d-block aspect-ratio aspect-ratio_60">
						<img
							src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>"
							width="<?=$arItem["PREVIEW_PICTURE"]["WIDTH"]?>"
							height="<?=$arItem["PREVIEW_PICTURE"]["HEIGHT"]?>"
		...
		<?if( isset($arFileValue) && $arResult["USER_HAVE_ACCESS"] ):?>
			<div class="text-center py-10 px-30 pt-xl-65 pb-xl-55">
				<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
					<a href="<?=$arFileValue["SRC"]?>" class="text-truncate-2 text-uppercase ff-dinpro fw-700 fz-16 fz-md-20 mb-5 mb-lg-15"><?echo $arItem["NAME"]?></a>
				<?endif;?>
			</div>
		<?else:?>
			<div class="text-center py-10 px-30 pt-xl-65 pb-xl-55">
		...
		<?foreach($arItem["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
			...
			<?elseif ($value != ""):?>
				<div class="fw-700 fz-15 fz-xl-20"><i class="fa"></i> <?=$arProperty["NAME"]?>:
					<?=$value;?>
				</div>
			<?endif;?>
		<?endforeach;?>
