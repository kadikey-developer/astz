...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<div class="border shadow-hover h-100">
		<?if($arParams["DISPLAY_PICTURE"]!="N"):?>
			...
			<?elseif (is_array($arItem["PREVIEW_PICTURE"])):?>
				<div class="text-center">
					<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
						<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="<? if ($arParams['ASPECT_RATIO'] == 'Y') echo 'd-block aspect-ratio aspect-ratio_60'; ?>">
							<img class="img-fluid"
								class="lazyload"
					...
					<?else:?>
						<div class="<?= ( $arParams['ASPECT_RATIO'] == 'Y' ? 'd-block aspect-ratio aspect-ratio_60' : 'my-20' ); ?>">
							<img class="img-fluid"
								src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>"
								<?/*width="<?=$arItem["PREVIEW_PICTURE"]["WIDTH"]?>"
								height="<?=$arItem["PREVIEW_PICTURE"]["HEIGHT"]?>"*/?>
		...
		<?else:?>
			<div class="text-center py-10 px-30 pt-xl-65 pb-xl-55">
				<div class="text-truncate-2 text-uppercase ff-dinpro fw-700 fz-16 fz-md-20 mb-5 mb-lg-15"><?=$arItem["NAME"]?></div>
