...
// попробовать переделать на комплексный компонент для работы в Режиме AJAX или разобраться как у news.list работает AJAX
?>
<?$APPLICATION->IncludeComponent(
	...
	Array(
		...
		"CONTROLS" => array(),
		"INIT_MAP_TYPE" => "MAP",
		"MAP_DATA" => serialize($arResult["yaMAP"]),
		"MAP_WIDTH" => "100%",
		"MAP_HEIGHT" => "300",
		"MAP_ID" => "",
...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	?>
	<div class="col-md-4 mb-30" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
	<div class="border shadow-hover h-100">
		<?if($arParams["DISPLAY_PICTURE"]!="N"):?>
			...
			<?elseif ($arItem["SLIDER"] ?? false):?>
				<div class="bx-newslist-img text-center">
		...
		<?endif;?>
		<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
			<div class="text-center py-10 px-30 pt-xl-65 pb-xl-55">
				<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
					<a href="<?echo $arItem["DETAIL_PAGE_URL"]?>" class="text-truncate-2 text-uppercase ff-dinpro fw-700 fz-16 fz-md-20 mb-5 mb-lg-15"><?echo $arItem["NAME"]?></a>
					<a class="fw-700 fz-15 fz-xl-20" href="<?echo $arItem["DETAIL_PAGE_URL"]?>"><?echo GetMessage("CT_BNL_GOTO_DETAIL")?></a>
				<?endif;?>
			</div>
		<?else:?>
			<div class="text-center py-10 px-30 pt-xl-65 pb-xl-55">
		...
		<?if($arParams["DISPLAY_PREVIEW_TEXT"]!="N" && $arItem["PREVIEW_TEXT"]):?>
			<div class="py-10 px-30 pt-xl-65 pb-xl-55">
		...
		<?foreach($arItem["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
			...
			<?elseif ($value != ""):?>
				<div class="px-30 bx-newslist-other"><i class="fa"></i><span class="ff-arial fw-700 fz-14"><?=$arProperty["NAME"]?>:</span>
					<span class="fz-12"><?=$value;?></span>
				</div>
			<?endif;?>
		<?endforeach;?>
